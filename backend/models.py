...
class Product(BaseModel):
    ...
    organization_id: str = ""
    # ---- Retailer-private SKU support (off-ecosystem brands) ---------------
    # ``source`` is the discriminator between ecosystem products (default —
    # browsed by all wholesalers/manufacturers) and retailer-private SKUs
    # which are only visible to the owning retailer. When source is
    # ``retailer_private``, ``owner_type`` is ``retailer`` and
    # ``owner_id`` is the retailer's entity_id. ``external_manufacturer``
    # is a free-text label for the off-ecosystem brand the retailer
    # bought from (e.g. "Mama Nkechi Local Snacks").
...
class RetailerPrivateProductCreate(BaseModel):
    """Retailer-owned product for off-ecosystem SKUs. All fields are
    retailer-supplied; nothing here is shared with manufacturers."""
    name: str = Field(min_length=1, max_length=160)
    category: str = Field(min_length=1, max_length=80)
    unit_price: float = Field(ge=0)
    sku: Optional[str] = None        # autogenerated if absent
    barcode: Optional[str] = None
    external_manufacturer: Optional[str] = Field(default=None, max_length=160)
    notes: Optional[str] = None
    client_op_id: Optional[str] = None


class RetailerPrivateProductUpdate(BaseModel):
