...
async def _spawn_vehicle(shipment: Dict[str, Any], rng: random.Random) -> Optional[Dict]:
    ...
    vehicle = {
        ...
        "created_at": now_iso(),
        "updated_at": now_iso(),
        # Track A schema-v2 flags so the new Dispatch UI knows to ignore simulator vehicles
        "source": "simulator",
        "owner_org_id": mfr,
...
async def ensure_factory_replenishment(rng: random.Random) -> int:
    ...
    async for m in db.organizations.find(
        ...
        await db.shipments.insert_one({
            ...
            "owner_org_id": mfr, "owner_org_type": "manufacturer",
            "items": items, "tracking_code": tracking,
            "status": "in_transit", "source": "simulator",
            "status_history": [
                {"from_status": None, "to_status": "created", "at": now,
                 "by_role": "system", "notes": "Factory replenishment (simulator)"},
                {"from_status": "created", "to_status": "in_transit", "at": now,
                 "by_role": "system", "notes": "Factory replenishment (simulator)"},
            ],
...
async def bridge_wholesaler_shipments() -> Dict[str, int]:
    ...
    async for po in db.wholesaler_purchase_orders.find(
        ...
        await db.shipments.insert_one({
            ...
            "owner_org_id": supplier_id, "owner_org_type": "distributor",
            "items": items, "tracking_code": tracking,
            "status": "in_transit", "source": "simulator",
            "status_history": [
                {"from_status": None, "to_status": "created", "at": now,
                 "by_role": "system", "notes": "Bridged from wholesaler PO"},
                {"from_status": "created", "to_status": "in_transit", "at": now,
                 "by_role": "system", "notes": "Bridged from wholesaler PO"},
            ],
    ...
    async for po in db.purchase_orders.find(
        ...
        await db.shipments.insert_one({
            ...
            "items": items, "tracking_code": tracking,
            "status": "in_transit", "source": "simulator",
            "owner_org_id": whlsr_id, "owner_org_type": "wholesaler",
            "status_history": [
                {"from_status": None, "to_status": "created", "at": now,
                 "by_role": "system", "notes": "Bridged from retailer PO"},
                {"from_status": "created", "to_status": "in_transit", "at": now,
                 "by_role": "system", "notes": "Bridged from retailer PO"},
            ],
