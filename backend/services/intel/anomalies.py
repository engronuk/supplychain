...
# Memory-bounded chunk size for large retailer fleets in production
RETAILER_BATCH = 200
...
async def detect_anomalies(tenant_id: str) -> dict:
    ...
    Memory-bounded: processes retailers in chunks of RETAILER_BATCH so the
    worker stays well below container limits. Yields to the event loop
    between chunks so healthchecks stay responsive.
    """
    ...
    if not retailer_ids_all:
        return {"alerts": 0}

    today = datetime.now(timezone.utc).date()
    start = (today - timedelta(days=20)).isoformat()

    # Replace old anomaly alerts for this tenant upfront so partial results
    # never leave stale + new mixed together.
    ...
    for chunk_start in range(0, len(retailer_ids_all), RETAILER_BATCH):
        ...
        rows = await db.daily_sales.find(
            {"retailer_id": {"$in": chunk_ids}, "date": {"$gte": start}},
            {"_id": 0, "retailer_id": 1, "date": 1, "units": 1},
        ).to_list(50_000)
        ...
        if alerts:
            await db.intel_alerts.insert_many(alerts, ordered=False)
            total_alerts += len(alerts)

        # Let healthchecks + other requests breathe
