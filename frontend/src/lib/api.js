...
export const Api = {
  ...
  wholesalerPendingOrders: (wholesalerId) =>
    api.get(`/logistics/wholesalers/${wholesalerId}/pending-orders`).then((r) => r.data),
  copilotHistory: () =>
...
// Auth is the standard super_admin JWT (same cookie/Bearer as the rest
// of the admin console) — no separate token needed.
export const SyncApi = {
  status: () => api.get("/admin/sync/status").then((r) => r.data),
  diff: () => api.post("/admin/sync/diff").then((r) => r.data),
  apply: (opts = {}) =>
    api.post("/admin/sync/apply", {
